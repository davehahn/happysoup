({
	...
	getUnassignedJobs : function(component) {
		var self = this,
				action = component.get('c.getUnassignedJobs'), la,
	      	...
	      	function(jobs) {
						...
						for (j of jobs) {
							...
							if (j.job.Placeholder__c)
								j.job.class="slds-box dragItem placeholder";
							else if (j.job.age > -1)
								j.job.class="slds-box dragItem dueLater";
							else if (j.job.age == -1)
								j.job.class="slds-box dragItem dueToday";
							else if (j.job.age < -1)
								j.job.class="slds-box dragItem dueDateMissed";
							else
								j.job.class="slds-box dragItem";
	...
	getCompletedJobs : function(component) {
		...
		var self = this,
				warehouse = component.get('v.city'),
				action = component.get('c.getCompletedJobs'), la,
	      	...
	      	function(jobs) {
						for (j of jobs) {
							if (j.job.Employee__c != null) {
								j.job.initials = j.job.Employee__r.User__r.FirstName.substr(0,1) + j.job.Employee__r.User__r.LastName.substr(0,1);
							}
							if (j.materials) {
			          for (m of j.materials) {
			            if (m.AcctSeedERP__Product__c != null && m.AcctSeedERP__Product__r.Family == 'Canvas') {
			...
			if ((search1 == null || search1 == '') && (search2 == null || search2 == '')) {
				...
				backlog.forEach(function(j) {
					if (j.hasOwnProperty('job')) {
						if (j.job.Manufactured_Product_Name__c != null 
								&& j.job.Manufactured_Product_Name__c.indexOf(search1) > -1) {
							results.push(j);
						} else if (j.job.hasOwnProperty('ERP_Order__r')) {
							if (j.job.ERP_Order__r.Model_Name__c != null 
									&& j.job.ERP_Order__r.Model_Name__c.indexOf(search1) > -1) {
								results.push(j);
							} else if (j.job.ERP_Order__r.Serial_Product__c != null 
												 && j.job.ERP_Order__r.Serial_Product__c.indexOf(search1) > -1) {
								results.push(j);
							} else if (j.job.ERP_Order__r.Manufactured_Product_Name__c != null 
												 && j.job.ERP_Order__r.Manufactured_Product_Name__c.indexOf(search1) > -1) {
								results.push(j);
							} else if (j.job.ERP_Order__r.AcctSeedERP__Manufactured_Product__r != undefined 
	...
	findJobs : function(component) {
		return new Promise(function(resolve, reject) {
			var action = component.get('c.findJobs'), la,
					query = component.get('v.backlogSearch'),
					keywords = [],
					cleanwords = [],
					filter = component.get('v.selectedBacklogFilter'),
