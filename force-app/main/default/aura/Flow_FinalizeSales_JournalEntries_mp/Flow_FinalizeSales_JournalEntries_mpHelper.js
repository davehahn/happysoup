({
	...
	getNames : function(component) {
		...
		if (f_2300a_who != null && f_2300a_who != '') {
			self.getUserName(f_2300a_who, component)
			.then(
		...
		if (f_2300b_who != null && f_2300b_who != '') {
			self.getUserName(f_2300b_who, component)
			.then(
	      $A.getCallback( function( result ) {
		...
		if (f_2300c_who != null && f_2300c_who != '') {
			self.getUserName(f_2300c_who, component)
			.then(
	      $A.getCallback( function( result ) {
		...
		if (f_2300d_who != null && f_2300d_who != '') {
			self.getUserName(f_2300d_who, component)
			.then(
	      $A.getCallback( function( result ) {
		...
		if (f_2300e_who != null && f_2300e_who != '') {
			self.getUserName(f_2300e_who, component)
			.then(
	      $A.getCallback( function( result ) {
	...
	getUserName : function(userId, component) {
		...
		var self = this,
				action = component.get('c.getUserName'), la;
		action.setParams({
			userId : userId
		});
		la = new LightningApex( this, action );
	...
	buildEntries : function(component) {
		...
		return new Promise( function(resolve, reject) {
			...
			b =  ( Math.round( (f_5420a*1) + (f_5420b*1) ) ) == Math.round( (f_2300a*1) + (f_2300b*1) + (f_2300c*1) + (f_2300d*1) + (f_2300e*1) );
	...
	createJournalEntry : function(component, entries) {
		...
		var self = this,
				action = component.get('c.createJournalEntry'), la,
	...
	getGLV4 : function(component, whoId) {
		var action = component.get('c.getGLV4'), la;
		return new Promise( function(resolve, reject) {
			action.setParams({
				"whoId" : whoId
			});
	...
	synchronize : function(component) {
		var f_1120			= parseFloat( component.get('v.f_1120') ),
				...
				f_2300c 		= parseFloat( component.get('v.f_2300c') ), f_2300c_hint,
				f_2300d 		= parseFloat( component.get('v.f_2300d') ), f_2300d_hint,
		...
		if (    ( ( f_5420a + f_5420b ) - ( f_2300a + f_2300b + f_2300c + f_2300d + f_2300e ) ) < 0.01
