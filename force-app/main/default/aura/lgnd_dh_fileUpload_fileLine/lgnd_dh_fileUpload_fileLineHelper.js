	...
	uploadFile : function( component, objectId )
  {
    ...
    return new Promise( function( resolve, reject) {
      ...
      fr.onload = $A.getCallback( function() {
        ...
        self.doUpload( component, objectId, file, fileContents, fromPos, toPos, '' )
        .then(
          ...
          action.setCallback(this, function( response ) {
            ...
            if (state === "SUCCESS")
            {
              ...
              else
              {
                if( isChunked )
                {
                  self.cleanUpChunckedFile( component, attachId, objectId )
                  .then(
                    $A.getCallback( function() {
                      resolve();
                    }),
            ...
            else if (state === "ERROR")
            {
              ...
              if (errors)
              {
                if (errors[0] && errors[0].message)
                  reject( 'helper line 134 ' + errors[0].message);//error[0].message );
              }
              else
              {
    ...
    return new Promise( function( resolve, reject ) {
      ...
      action.setCallback(this, function( response ) {
        ...
        else if (state === "ERROR")
        {
          ...
          {
            if (errors[0] && errors[0].message)
              reject( 'helper line 184');//error[0].message );
          }
          else
          {
