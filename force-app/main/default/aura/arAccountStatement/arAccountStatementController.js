({
	doInit: function(component, event, helper) {
        ...
        component.set('v.otherColumns', [
          {label: 'Name', fieldName: 'Name', type: 'text'},
          {label: 'Balance', fieldName: 'AcctSeed__Balance__c', type: 'currency', typeAttributes: { currencyCode: 'CAD'}}
        ]);
        component.set('v.erpColumns', [
          {label: 'Name', fieldName: 'Name', type: 'text'},
          {label: 'Grand Total', fieldName: 'grandTotal', type: 'currency', typeAttributes: { currencyCode: 'CAD'}},
          {label: 'Total Billed', fieldName: 'billTotal', type: 'currency', typeAttributes: { currencyCode: 'CAD'}},
          {label: 'Unbilled', fieldName: 'erpUnbilled', type: 'currency', typeAttributes: { currencyCode: 'CAD'}}
        ]);
    ...
    rowSelected: function( component )
    {
        ...
        if(billTable != undefined && billTable.length > 1){
          for(var i = billTable.length; i == 1; i--)
            billTable[i].destroy();
    ...
    receiveBillings: function(component, event, helper) {
        ...
        if(billTable != undefined && billTable.length > 1){
          table = billTable[1];
            ...
            table = billTable;
        var billIds = [];
        for( var bill of table.getSelectedRows() )
