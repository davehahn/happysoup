({
  ...
  calculateInsuranceTotal: function( component )
  {
    ...
    if( insuranceItems !== undefined && insuranceItems !== null)
    {
      ...
      {
        if( insuranceItems[i].termError )
          component.set('v.termError', true);
  ...
  calculatePayments: function( component, hasChanged )
  {
    ...
    {
      payment = ( compoundIntrest * amountFinanced ) / ( 1 - ( Math.pow( (1 + compoundIntrest), (- ( finAmort * compound_per_year / 12 ) ) ) ) );
      remaining = amountFinanced *( Math.pow((1+ compoundIntrest ), ( finTerm * compound_per_year / 12)) ) -( payment )*( ( Math.pow((1+ compoundIntrest ),( finTerm * compound_per_year / 12)) - 1 ) /compoundIntrest );
    }
