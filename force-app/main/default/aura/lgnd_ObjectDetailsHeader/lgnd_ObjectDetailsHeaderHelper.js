({
	...
	fetchRecord : function( component )
  {
    ...
    if( canEditFormula !== undefined &&
        canEditFormula !== null &&
        canEditFormula.length > 0 )
    {
    ...
    for( var i=0; i<criteria.length; i++ )
    {
      ...
      if( criteriaArray.length === 3 )
      {
        ...
        switch( oper )
        {
          ...
          case 'CONTAINS':
            if( recordValue.includes( matchValue ) )
              result = true;
            break;
        }
      }
  ...
  setDisplayFields: function( component )
  {
    ...
    if( !$A.util.isEmpty( displayFieldsString ) )
    {
      fields = displayFieldsString.split(',').reduce( function( flds, fieldString ) {
        ...
        if( field.length > 1 )
        {
          var format = ( field[2] === undefined || self.validFormats.indexOf( field[2] ) < 0  ) ?
            'STRING' : field[2]
          flds.push({
            label: field[0],
