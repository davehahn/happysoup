({
  ...
  {
    var action = component.get('c.fetchPricebook'), la;
    action.setParams({
  ...
  {
    var action = component.get('c.clonePricebook'), la,
        params = {
  ...
  successToastMessage: function( component )
  {
    ...
    else
    {
      ...
      if( lastComma > 0 )
        title = title.substring(0, lastComma) + ' and' + title.substring(lastComma+1);
      title += '....';
    }
    LightningUtils.showToast('success',
                              title,
                              message);
  }
