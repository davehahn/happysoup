...
export default class FactoryStoreCurrentInventoryList extends NavigationMixin(LightningElement) {
  ...
  connectedCallback() {
    this.startingRetailPrice = this.isEN
      ? formatPrice(this.boat.Expanded.RetailPrice, true)
      : formatPrice(this.boat.Expanded.RetailPrice, true, "fr");
    this.startingWeeklyPrice = this.isEN
      ? weeklyPayment(this.boat.Expanded.RetailPrice)
      : weeklyPayment(this.boat.Expanded.RetailPrice, "fr");
    ...
    this.standardMotor = this.boat.Base.Standard_Motor__r
      ? rewriteMotorName(this.boat.Base.Standard_Motor__r.Name)
      : "";
    if (this.isEN) {
      this.standardTrailer = this.boat.Base.Standard_Trailer__r
        ? " and " + rewriteTrailerName(this.boat.Base.Standard_Trailer__r.Name)
        : "";
    } else {
      this.standardTrailer = this.boat.Base.Standard_Trailer__r
        ? " et " + rewriteTrailerName(this.boat.Base.Standard_Trailer__r.Name)
        : "";
    }
    ...
    const inventory = fetchNewInStockInventory({
      location: parseLocationName(this.locationName),
      year: this.modelYear,
      modelId: this.boatId
    })
      .then((boats) => {
          ...
          boats.RiggedBoats.forEach((boat, index) => {
            ...
            if (boat.Equipment) {
              ...
              boat.Equipment.forEach((e, i) => {
                ...
                let productName =
                  e.productType === "Motor" ? rewriteMotorName(e.productName) : rewriteTrailerName(e.productName);
              ...
              this.storeStock[index].Base.SpecialRetailPrice =
                this.pBoat.Expanded.RetailPrice + this.storeStock[index].Base.Savings + retailUpgradeCost;
              ...
              this.storeStock[index].Base.HasSpecialPrice = this.storeStock[index].Base.Savings < 0 ? true : false;
              this.storeStock[index].Base.SpecialRetailPrice =
                this.pBoat.Expanded.RetailPrice + this.storeStock[index].Base.Savings;
  ...
  formatListingPrices(list) {
    list.forEach((boat, index) => {
      boat.Base.StartingWeeklyPrice = this.isEN
        ? weeklyPayment(boat.Base.StartingRetailPrice)
        : weeklyPayment(boat.Base.StartingRetailPrice, "fr");
      boat.Base.StartingRetailPrice = this.isEN
        ? formatPrice(boat.Base.StartingRetailPrice, true)
        : formatPrice(boat.Base.StartingRetailPrice, true, "fr");

      boat.Base.SpecialWeeklyPrice = this.isEN
        ? weeklyPayment(boat.Base.SpecialRetailPrice)
        : weeklyPayment(boat.Base.SpecialRetailPrice, "fr");
      boat.Base.SpecialRetailPrice = this.isEN
        ? formatPrice(boat.Base.SpecialRetailPrice, true)
        : formatPrice(boat.Base.SpecialRetailPrice, true, "fr");
    });
    return list;
  }
