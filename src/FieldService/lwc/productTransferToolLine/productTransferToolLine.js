...
export default class ProductTransferToolLine extends LightningElement {
  ...
  @api transferSuccess(quantity) {
    ...
    return new Promise((resolve, reject) => {
      if (
        (this.transferLine.quantityRequired === quantity && this.showRequired) ||
        ...
        if (this.showRequired) this.transferLine.quantityRequired = this.transferLine.quantityRequired - quantity;
  ...
  get canSubmit() {
    return !(
      Object.keys(this.transferLine).indexOf("quantityTransferred") >= 0 &&
      this.transferLine.quantityTransferred > 0 &&
      this.transferLine.quantityTransferred <= this.transferLine.quantityAvailable &&
      ((this.transferLine.quantityTransferred <= this.transferLine.quantityRequired && this.showRequired) ||
        (this.transferLine.quantityTransferred <= this.transferLine.quantityAvailable && !this.showRequired))
    );
  }
