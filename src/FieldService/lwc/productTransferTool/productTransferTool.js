...
export default class FslTransferTool extends LightningElement {
  ...
  get technicianName() {
    return this._locations.employeeLocations !== null && this._locations.employeeLocations.length === 1
      ? this._locations.employeeLocations[0].Name
      : "Technician";
  }

  get showBinLocations() {
    return this._sourceLocation !== null && this._sourceLocation.LocationType === "Warehouse";
  }
  ...
  get showStandardTable() {
    return this.products !== null && this.products.length > 0 && this.transferType !== "return";
  }

  get showReturnTable() {
    const r = this.products !== null && this.products.length > 0 && this.transferType === "return";
    return r;
  }

  get showWoliSelector() {
    return this.woliSelectOptions !== undefined && this.woliSelectOptions.length > 0 && this.selectedWOLI === undefined;
  }
  ...
  handleDistribute() {
    if (this._locations.employeeLocations.length === 0) {
      ...
      if (this._locations.employeeLocations.length === 1) {
        ...
        this.destinationOptions = this._locations.employeeLocations.reduce((acc, loc) => {
          acc.push({
            label: loc.Name,
            value: loc.Id,
  ...
  handleReturn() {
    ...
    this.sourceSelectLabel = "Select Item's Current Location";
    this.sourceOptions = this._locations.employeeLocations.reduce((acc, loc) => {
      acc.push({
        label: loc.Name,
        value: loc.Id,
        type: loc.LocationType
  ...
  handleLocationChange(event) {
    ...
    if (this._sourceLocation && this._destinationLocation) {
      ...
      if (this.transferType === "return") this.fetchProductListForReturn();
  ...
  async handleSingleTransfer(event) {
    ...
    try {
      ...
      if (completeTransfer) this.products = this.products.filter((p) => p.id !== value.id);
      if (this.products.length === 0) this.handleAllTransfersComplete();
    } catch (err) {
      this.toggleSpinner(false);
      errorToast(this, reduceErrors(err)[0]);
    }
  ...
  toggleSpinner(open) {
    if (this._spinner === undefined) this._spinner = this.template.querySelector("c-legend-spinner");
