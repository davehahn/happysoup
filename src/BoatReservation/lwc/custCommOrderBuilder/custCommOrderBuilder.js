...
export default class CustCommOrderBuilder extends NavigationMixin(LightningElement) {
  ...
  renderedCallback() {
    loadStyle(this, sldsIconFont + "/style.css").then(() => {});
  ...
  get processPages() {
    return this.pages.map((page) => {
      return {
        ...
        class: this.currentPage === page.label ? "config-nav-item config-nav-item_selected" : "config-nav-item"
      };
    });
  }
  ...
  handleCustomerData(event) {
    ...
    if (attr === "state") this.handleFreight(value);

    if (attr === "firstName") this.customerFirstName = value;

    if (attr === "lastName") this.customerLastName = value;
  }
  ...
  doModalPageChange(page) {
    ...
    this.template
      .querySelector(`[data-modal-nav-name="${this.currentModalPage}"]`)
      .classList.add("modal-nav-item_selected");
  ...
  jumpToPayment() {
    this.onPaymentPage() ? this.submitOrder() : this.doPageChange("payment");
  }
  ...
  doPageChange(page) {
    ...
    if (this.currentPage === "payment") {
      this.template.querySelector('[data-id="square-payment-container"]').src =
        "/apex/Square_PaymentForm_CustomerCommunity";
    }
  }
  ...
  createSquarePayment() {
    return this.template.querySelector("c-square-payment-form").doPostToSquare(this.paymentAmount, this.opportunityId);
    /* for thank you testing */
    //    return Promise.resolve();
  }
  ...
  saveSaleItems() {
    ...
    let lineItems = this.performanceItems.concat(
      this.traileringItems,
      this.electronicsItems,
      boatLineItem,
      premiumPackLineItem
    );
  ...
  displayFreightCharge(charge) {
    ...
    this.freightCharge = this.isEN
      ? "+ " + updatedFreight + " Freight Charge"
      : "+ " + updatedFreight + " Frais de transport";
  }
  ...
  validateField(field) {
    ...
    if (attr === "email") {
      if (value.length === 0) {
        ...
        if (!pattern.test(value)) {
          let errmsg = this.isEN
            ? "This field is in the wrong format. Try using the format <em>email@address.com</em> instead."
            : "Ce champ est en mauvais format.  Essayez d'utiliser ce format à la place <em>email@address.com</em> ";
          feedback.classList.remove("clean");
          feedback.classList.add("error");
          feedback.innerHTML = errmsg;
          field.classList.remove("clean");
    ...
    if (attr === "phone") {
      if (value.length === 0) {
        ...
        if (!pattern.test(value)) {
          let errmsg = this.isEN
            ? "This field is in the wrong format. Try using the format <em>123-456-7890</em> instead"
            : "Ce champ est en mauvais format.  Essayez d'utiliser ce format à la place  <em>123-456-7890</em>";
          feedback.classList.remove("clean");
          feedback.classList.add("error");
          feedback.innerHTML = errmsg;
          field.classList.remove("clean");
