...
export default class CustCommOrderOptions extends LightningElement {
          ...
          options.forEach((option, index) => {
            if (option !== null) {
              if ("id" in option) {
                ...
                if (parsedOption !== undefined) combinedOptions.push(parsedOption[0]);
              } else {
                //console.log('no id');
              }
            }
  ...
  parseOption(option, init) {
    ...
    let detailedSummary_fr = null;
    let includedProducts = [];

    const retailPrice =
      "retailPrice" in option
        ? parseInt(option["retailPrice"])
        : "RetailUpgradeCost" in option
        ? parseInt(option["RetailUpgradeCost"])
        : "";
    let displayPrice = 0;
    ...
    if ("marketingContent" in option) {
      ...
      for (const mc of marketingContent) {
        ...
        } else if (label === "detailedsummary_fr") {
          detailedSummary_fr = origContent;
        }
      }
    }
    ...
    optionDetails.push({
      ...
      detailedSummary_fr: detailedSummary_fr
    });
