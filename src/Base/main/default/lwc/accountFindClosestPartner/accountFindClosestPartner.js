...
export default class AccountFindClosestPartner extends LightningElement {
  ...
  handleMarkerSelect(event) {
    let selectedAcct = this.mapMarkers.filter((marker) => marker.id === event.target.selectedMarkerValue)[0];
    this.dispatchEvent(new CustomEvent("accountselected", { detail: selectedAcct }));
  }
