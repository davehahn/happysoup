...
export default class ErpCreatePartsBackorder extends LightningElement {
  ...
  @api stageTransferredMaterials() {
    return new Promise((resolve, reject) => {
      this.transferredMaterials = Array.from(this.materials.values()).filter((mat) => mat.quantityTransferred > 0);
      resolve(1);
    });
  }
  ...
  _determineProjectTaskForRetail() {
    return this.retailERP.AcctSeed__Project_Tasks__r.filter((task) => task.Is_Retail_Boat_Sale__c)[0];
  }
