...
export default class CasePartsLinkPol extends LightningElement {
  ...
  groupPOLsToCaseParts() {
    ...
    this.caseParts.forEach((casePart) => {
      let item = {
        ...
        availablePOLs: this.purchaseOrderLines.filter(
          (pol) => casePart.Product__c === pol.productId && pol.qtyRemaining >= casePart.Quantity__c
        )
      };
  ...
  handlePolSelect(event) {
    ...
    doLink(dataSet)
      .then((result) => {
        this.dispatchEvent(new CustomEvent("linksuccess", { detail: result }));
      })
      .catch((err) => {
        this._ready = true;
  ...
  handleCreateNew() {
    ...
    console.log(params);
    this.dispatchEvent(new CustomEvent("createnew", { detail: params }));
  }
}
