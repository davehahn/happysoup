...
({
  ...
  afterScripts: function (component, event, helper) {
    const spinner = component.find("spinner"),
      partnerSteps = ["Select Customer", "Submit Promotion", "Upload Documentation", "Finish"],
      factorySteps = ["Select Retail ERP", "Finish"];
    spinner.toggle();
    helper
      .init(component)
      .then(
  ...
  handleSelectPromo: function (component, event, helper) {
    ...
    if (
      Object.keys(promo.Promotion__r).indexOf("Document_Requirements__c") >= 0 &&
      promo.Promotion__r.Document_Requirements__c !== undefined &&
      promo.Promotion__r.Document_Requirements__c.length > 0
    ) {
      component.set("v.requiredPromotionDocuments", promo.Promotion__r.Document_Requirements__c.split(";"));
      requiresDocuments = true;
    }

    spinner.toggle("Creating Promotion Case");
    helper
      .createPromoCase(component, promo)
      .then(
        $A.getCallback((result) => {
