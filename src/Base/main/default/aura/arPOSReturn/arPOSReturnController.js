({
	...
	handleFilterSelected: function( component, event, helper )
	{
		...
		// return;
		helper.toggleSpinner(component, true);
		helper.runAction(component, "c.retrieveERPOrders", {
			idFilter: idFilter
		}, function(response) {
			...
			if(results.length > 0)
			{
				...
				helper.showToast(component, "Error", "error", "The Selected Filter does not have any ERP Order Items to return.");
			}
			helper.toggleSpinner(component, false);
		});
	},
	...
	processReturn: function( component, event, helper )
  {
	...
	processReturn_Old: function( component, event, helper )
	{
		...
		helper.runAction(component, "c.saveReturnMaterials", {
	...
	{
		if(component.get('v.materials').length > 0)
			helper.returnAllMaterials( component, event, helper );
		else
			helper.showToast(component, "Error", "error", "Please select some ERP Order Items to return.");
	},
