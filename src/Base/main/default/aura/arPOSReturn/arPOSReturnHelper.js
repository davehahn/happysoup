({
  ...
  returnThisProduct: function (component, event, helper) {
    ...
    if (addedMaterials.indexOf(selectedMaterial.matId) === -1) {
      ...
      if (selectedMaterial.serialId != null) returnMatl.serialNumber = selectedMaterial.serialName;
  ...
  processRefundStart: function (component, event, helper) {
    ...
    self.processBilling(component).then(
  ...
  processRefundPayment: function (component, event, helper) {
    ...
    var self = this;
    self.processPayment(component).then(
      $A.getCallback(function (response) {
        var results = response;
          ...
          if (errors) {
            ...
            if (errors[0] && errors[0].message) {
              ...
              helper.showToast(
                ...
                "Error In COGS",
                "error",
                "The Return was completed, but the COGS movement was not done. Please post the COGS Journal entry from ERP order page."
              );
            }
            return;
          }
  ...
  calculateTotalAmount: function (component, event) {
    ...
    if (quantity > tmpMaterialList[index].matQuantity || quantity <= 0 || tmpMaterialList[index].serialized) {
      ...
      tmpMaterialList[index].totalPrice = (
        tmpMaterialList[index].matQuantity * tmpMaterialList[index].pricePerQuantity
      ).toFixed(2);
  ...
  returnAllMaterials: function (component, event, helper) {
    ...
    component.set(
      "v.cssCustomStyle",
      ".forceStyle .viewport .oneHeader.slds-global-header_container {z-index:0} .forceStyle.desktop .viewport{overflow:hidden}"
    );
  },
  closeModal: function (component, event, helper) {
    ...
    component.set(
      "v.cssCustomStyle",
      ".forceStyle .viewport .oneHeader.slds-global-header_container {z-index:5} .forceStyle.desktop .viewport{overflow:visible}"
    );
  },
    ...
    empApi
      .subscribe(
        ...
        $A.getCallback((eventReceived) => {
          ...
          if (eventReceived.data.payload.Status__c === "success" && eventReceived.data.payload.ERP_Order__c === erpID)
