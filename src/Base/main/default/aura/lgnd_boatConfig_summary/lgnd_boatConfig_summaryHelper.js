({
  ...
  updateTotal: function (component) {
      ...
      sumOptions = function (options) {
        return options.reduce((optTotal, option) => {
          ...
          if (option.isCheckbox != true && parseFloat(option.quantitySelected) > 0) {
            optTotal +=
              (parseFloat(option.quantitySelected) - parseFloat(option.quantityStandard)) *
              parseFloat(option.individualCost);
          }
    ...
    if (boat !== null && boat.length !== 0 && boat.cost !== undefined) total += boat.cost;
    ...
    if (
      trollingMotor !== undefined &&
      trollingMotor !== null &&
      trollingMotor.length !== 0 &&
      trollingMotor.cost !== undefined &&
      !isNaN(trollingMotor.cost)
    ) {
