({
  ...
  doToggleCreateQuote: function (component) {
    ...
    if (opportunityId !== undefined && opportunityId !== null && opportunityId.length > 0) {
      component.set("v.recordId", opportunityId);
      //set the quote name to default to BoatName / MotorName
      if (cpq.theBoat !== undefined && cpq.theBoat !== null) quoteName += cpq.theBoat.name;

      if (cpq.theMotor !== undefined && cpq.theMotor !== null) quoteName += " / " + cpq.theMotor.name;
  ...
  saveCPQ: function (component, quoteId) {
    var action = component.get("c.saveCPQ"),
      recordId = component.get("v.recordId"),
      cpq = component.get("v.cpq");
    if (quoteId !== undefined && quoteId !== null) cpq.saveToRecordId = quoteId;

    if (cpq.saveToRecordId === null || cpq.saveToRecordId === undefined || cpq.saveToRecordId.length === 0) {
