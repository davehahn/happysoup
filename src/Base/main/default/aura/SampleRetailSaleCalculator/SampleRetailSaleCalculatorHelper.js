({
  ...
  fetchMajorProductDetails: function (component, family, recordType, productId) {
    ...
    if (typeof productId !== "string") {
      if (typeof cpq.boatId === "string") self.unSelectMajorProduct(component);
    } else {
    ...
    add: {
      ...
      checkboxSubOption: function (selectedOpts, option) {
        for (let sOpt of selectedOpts) {
          if (option.parentProductId === sOpt.id) sOpt.selectedSubOptions.push(option);
        }
      },
      multipleOption: function (selectedOpts, option) {
        var idx;
        for (var i = 0; i < selectedOpts.length; i++) {
          if (option.id === selectedOpts[i].id) idx = i;
        }
        if (isNaN(idx)) selectedOpts.push(option);
        else selectedOpts[idx].quantitySelected = option.quantitySelected;
      },
      multipleSubOption: function (selectedOpts, option) {
        var parentIdx, subIdx;
        for (var i = 0; i < selectedOpts.length; i++) {
          if (option.parentProductId === selectedOpts[i].id) parentIdx = i;
        }
        if (!isNaN(parentIdx)) {
          for (var i = 0; i < selectedOpts[parentIdx].selectedSubOptions.length; i++) {
            if (option.id == selectedOpts[parentIdx].selectedSubOptions[i].id) subIdx = i;
          }
        }
        if (isNaN(subIdx)) selectedOpts[parentIdx].selectedSubOptions.push(option);
        else selectedOpts[parentIdx].selectedSubOptions[subIdx].quantitySelected = option.quantitySelected;
      }
    },
    remove: {
      option: function (selectedOpts, option) {
        ...
        if (idx != null) selectedOpts.splice(idx, 1);
      },
      subOption: function (selectedOpts, option) {
        ...
        if (!isNaN(subIdx)) selectedOpts[parentIdx].selectedSubOptions.splice(subIdx, 1);
      }
    }
  },
  ...
  calcPartnerMotorCost: function (component) {
    ...
    if (cpq.theMotor == null) component.set("v.calcdPartnerMotorCost", null);
    else component.set("v.calcdPartnerMotorCost", cpq.theMotor.partnerPrice - cpq.theMotor.partnerPrice * mercDiscount);
  },
  ...
  calcFees: function (obj, amountField) {
    ...
    if (obj.fees != null && Object.keys(obj).indexOf("fees") >= 0 && typeof obj.fees !== "undefined") {
      total = obj.fees.reduce(function (t, fee) {
        if (!isNaN(parseFloat(fee[amountField]))) t += parseFloat(fee[amountField]);
        return t;
      }, total);
    }
    return total;
  ...
  calcPartnerTotal: function (component) {
    ...
    if (isNaN(promo)) promo = 0;
    total += promo;
    if (coopD != null) total += coopD;
    if (volumeD != null) total += volumeD;
