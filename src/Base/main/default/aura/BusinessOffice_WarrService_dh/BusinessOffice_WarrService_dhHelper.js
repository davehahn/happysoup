({
  ...
  {
    var action = component.get('c.initServiceAndWarranty'), la;
    ...
    if(options !== undefined && options !== null )
    {
      for( var i = 0; i < options.length; i++ )
      {
        if( options[i].get('v.isSelected') )
        {
          ...
          if( options[i].get('v.lineId') !== null )
            currentLineIds.push( options[i].get('v.lineId') );
          ...
          if( subOpts !== undefined && subOpts !== null && subOpts.length > 0 )
          {
            for( var ii = 0; ii < subOpts.length; ii++ )
            {
              if( subOpts[ii].isSelected )
              {
                ...
                if( subOpts[ii].lineId !== null )
                  currentLineIds.push( subOpts[ii].lineId );
              }
            }
          }
        }
