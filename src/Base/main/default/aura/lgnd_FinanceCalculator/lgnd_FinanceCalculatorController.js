({
  ...
  reInit: function (component, event, helper) {
    ...
    if (initd) helper.initComponent(component);
  },
  ...
  rate_deposit_change: function (component, event, helper) {
    ...
    else if (deposit < 0) LightningUtils.errorToast("Deposit can not be less then zero");
    else {
      changeEvt
        .setParams({
          finTerm: finTerm,
  ...
  remoteUpdate: function (component, event, helper) {
    ...
    return new Promise(function (resolve, reject) {
      if (hasChanged) {
        helper.doRecordSave(component).then(
