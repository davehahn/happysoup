({
  ...
  clickSerialNumber: function (component, event) {
    ...
    return new Promise(function (resolve, reject) {
      if (skipCard) {
        self.fillNest(component, event, serno, sernoId, nestId).then(function () {
          console.log("resolve?");
          resolve();
        });
      } else {
        self.populateSerialCard(component, event, serno, sernoId, nestId).then(function () {
          console.log("resolve?");
          resolve();
        });
      }
  ...
  populateSerialCard: function (component, event, serno, sernoId, nestId) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      details.setCallback(this, function (response) {
        ...
        if (
          productDetails[0].Registrations__r !== undefined &&
          productDetails[0].Registrations__r[0].Account__r !== undefined
        ) {
