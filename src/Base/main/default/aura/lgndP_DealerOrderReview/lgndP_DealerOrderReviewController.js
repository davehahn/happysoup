({
  ...
  addToOrder: function (component, event, helper) {
    ...
    nav.setParams({ firedBy: 2, navigateTo: 1 });
    nav.fire();
  },
  ...
  applyPartnerProgram: function (component, event, helper) {
    var nav = $A.get("e.c:lgndP_DealerOrderNav_Event");
    nav.setParams({ firedBy: 2, navigateTo: 3 });
    nav.fire();
  },
  ...
  handleTableAction: function (component, event, helper) {
    ...
    if (action == "edit") helper.editOrderRow(component, groupId);
    if (action == "view") helper.viewOrderRow(component, groupId);
  },
