({
  doInit: function (component, event, helper) {
    ...
    fieldDetailsMap = fields === undefined || fields === null ? [] : fields.split(",");
    ...
    if (lists !== undefined) {
      ...
      if (!localConfig.sortDir) localConfig.sortDir = "DESC";
      ...
      if (localConfig.perPage) component.set("v.perPage", localConfig.perPage);
    }

    //setup table
    if (!localConfig.sortField) localConfig.sortField = fieldDetails[0].apiName;

    component.set("v.sortField", localConfig.sortField);

    if (!localConfig.sortedCol) localConfig.sortedCol = fieldDetails[0].name;
  ...
  handleSearch: function (component, event, helper) {
    ...
    if ((searchString === null || searchString.length === 0 || searchString.length > 3) && !isLoading) {
      setTimeout(() => {
        if (component.get("v.searchInputString") === searchString) component.set("v.searchString", searchString);
      }, 500);
    }
  },
  ...
  handleTableAction: function (component, event, helper) {
    ...
    if (action == "edit") helper.editRow(component, recordId);
    if (action == "view") helper.viewRow(component, recordId);
  },
