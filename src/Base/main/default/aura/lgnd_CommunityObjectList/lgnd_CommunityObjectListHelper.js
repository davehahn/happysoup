({
      ...
      addValueToObj = function (obj, base) {
        return Object.keys(obj).reduce((clone, key) => {
          key
            .split(".")
            .reduce(
              (innerObj, innerKey, i, arr) =>
                (innerObj[innerKey] = i + 1 === arr.length ? obj[key] : innerObj[innerKey] || {}),
              clone
            );
          return clone;
        }, Object.assign({}, base));
  ...
  fetchRecords: function (component) {
    ...
    for (var i = 0; i < fieldDetails.length; i++) {
      ...
      if (fieldDetails[i].type === "STRING") searchFields.push(fieldDetails[i].apiName);
    }
    ...
    if (globalFilter !== undefined && globalFilter.length > 0) params.listFilter = globalFilter;

    if (listFilter !== undefined && listFilter.length > 0) {
      if (params.listFilter !== null) params.listFilter += " AND " + listFilter;
      else params.listFilter = listFilter;
    }

    if (useCache) action.setStorable();
    else {
