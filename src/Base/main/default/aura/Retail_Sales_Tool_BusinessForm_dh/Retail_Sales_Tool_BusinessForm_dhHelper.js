({
  getSelectOptions: function (component) {
    var action = component.get("c.buildSelectOptions"),
      la;
  ...
  fetchBusiness: function (component, objectId) {
    ...
    var action = component.get("c.fetchBusiness"),
      la;
    action.setParams({
      objectId: objectId
    });
  ...
  formValid: function (component, stepNum) {
    ...
    return requiredFields.reduce(function (validSoFar, inputCmp) {
      ...
      } else return validSoFar && inputCmp.get("v.validity").valid;
    }, true);
  },

  verifyContactBuyerSet: function (component) {
    for (let c of component.get("v.business").contacts) {
      if (c.contactRoleType && c.contactRoleType === "buyer") return true;
    }
    return false;
  },
  ...
  createSale: function (component) {
    ...
    if (oppId === undefined) {
      ...
      var action = component.get("c.createSale_Business"),
        la;
      action.setParams({
