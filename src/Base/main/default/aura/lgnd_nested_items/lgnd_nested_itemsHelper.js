    ...
    if (item.GMBLASERP__Product__r.Can_be_Nested__c) {
      ...
      if (component.get("v.stage") == "replace") {
        if (nest[i].Product_Record_Type__c == item.Product_Record_Type__c) {
          ...
          component.set(
            "v.errorMessage",
            "You can only replace a " + item.Product_Record_Type__c + " with another " + item.Product_Record_Type__c
          );
          return false;
        }
      } else {
        ...
        if (count == 0) {
          ...
          component.set(
            "v.errorMessage",
            "There can only be 1 " + item.Product_Record_Type__c + " per boat at a time."
          );
          return false;
        }
      }
