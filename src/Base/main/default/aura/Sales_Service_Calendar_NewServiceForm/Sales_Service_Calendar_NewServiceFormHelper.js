({
  ...
  storeTask: function (component) {
    ...
    if ((task.name !== undefined && task.name !== null) || (task.complaint !== undefined && task.complaint != null)) {
      if (taskNumber === undefined || taskNumber === null) {
        ...
        if (pills.length === undefined) pills.getElement().classList.remove("selected");
        else {
          for (let p of pills) {
            var e = p.getElement();
  ...
  getPrepaidsOnAccount: function (component) {
    var accountId = component.get("v.accountId"),
      action = component.get("c.getPrepaidsOnAccount"),
      la;
    action.setParams({
      accountId: accountId
    });
