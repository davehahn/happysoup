({
  ...
  {
    var action = component.get('c.insurance_init'), la;
    action.setParams({
  ...
  buildItemsToSend: function( component )
  {
    ...
    if( Object.keys( selected ).length > 0 )
    {
      for( let i of Object.keys( selected ) )
      {
        ...
        {
          if( result.idsToDelete.indexOf( selected[i].recordId ) < 0 )
            result.idsToDelete.push( selected[i].recordId);
        }
      }
    }
    return result;
  ...
  recalculate: function( component, sendChangeEvent )
  {
    ...
    var self = this, newChangedItems, changeEvt;
    self.getSelectedItems( component )
    .then(
  ...
  calculateCost: function( selected, component )
  {
    ...
    return LightningUtils.doInsuranceCostCalculations( selected,
                                                       preInsuranceAmount,
                                                       deposit,
                                                       term,
                                                       amort,
  ...
  updateInsuranceItemAttr: function( data, attr, component )
  {
    ...
    if( items !== undefined && items.length > 0 )
    {
      for( var i=0; i<items.length; i++ )
      {
        ...
        if( data.hasOwnProperty( prodId ) )
          items[i].set(attr, data[prodId] );
      }
    }
  }
})