({
        ...
        if (leadId !== undefined && leadId !== null) {
          ...
          helper.fetchCustomer(component, leadId).then(
            $A.getCallback(function (result) {
              ...
              if (
                Object.keys(result).indexOf("leadCampaignInfluences") >= 0 &&
  ...
  handleSaveAfterAccountSelection: function (component, event, helper) {
    ...
    var AccIds = [];
    for (var acc of table.getSelectedRows()) AccIds.push(acc.Id);
  ...
  rowSelected: function (component) {
    ...
    var table;
    if (dataTable != undefined && dataTable.length > 1) {
      for (var i = dataTable.length; i == 1; i--) dataTable[i].destroy();
