({
  ...
  {
    var action = component.get('c.fetchInsurancePlanItem'), la;
  ...
  findCurrentRate: function( component )
  {
    ...
    for( let r of rates )
    {
      if( 
          ...
          (
            ...
            term <= r.Upper_Term__c && 
            (
              r.Lower_Finance_Term__c == null ||
              r.Upper_Finance_Term__c == null
            )
          )
        )
      {
    ...
    if( rateLine !== undefined )
    {
      ...
      if( coverage === 'Single' )
        component.set('v.insuranceRate', rateLine.Single__c);
      else if( coverage === 'Joint' ) {
        if (isFromQuebec)
          component.set('v.insuranceRate', rateLine.Joint_Factored_Result__c);
        else
          component.set('v.insuranceRate', rateLine.Joint__c);
      }
      else
        component.set('v.insuranceRate', null);
    }
    else
    {
