({
  setUserType: function (component) {
    ...
    return new Promise(function (resolve, reject) {
      self.actionHandler.call(self, component, action, true).then(
        $A.getCallback(function (result) {
          ...
          if (result.uiTheme !== "Theme3") component.set("v.inCommuninty", false);
          resolve();
        })
      );
    });
  ...
  saveDealerOrderLine: function (component) {
    var self = this,
      ...
      findSelectedOptions = function (opts) {
        return opts.reduce((result, opt) => {
          if ((opt.isCheckbox && opt.isSelected) || (!opt.isCheckbox && opt.quantitySelected > 0)) {
            result.push(opt);
          }
    ...
    for (var i = 0; i < fees.length; i++) {
      if (!isMotorRequest) erpLineItems.push(fees[i]);
    }
    data.discounts = [];
    ...
    if (motor !== null && Object.keys(motor).length > 0) {
      ...
      } else if (!isMotorRequest && (dealerOrder.Is_Legend_Transfer__c || order_ProductRecordType === "Motor")) {
        data.motor = motor;
        erpLineItems.push(motor);
        if (motor.optionalProducts !== undefined && motor.optionalProducts !== null) {
          erpLineItems = erpLineItems.concat(findSelectedOptions(motor.optionalProducts));
    ...
    return new Promise(function (resolve, reject) {
      self.handleMotorRequest
        .call(self, component, motorRequest)
        .then(
          $A.getCallback(function (result) {
            motorRequest = result;
            //required for saving
  ...
  handleMotorRequest: function (component, motorRequest) {
    ...
    return new Promise(function (resolve, reject) {
      if (
        (motorRequest.Motor__c === undefined || motorRequest.Motor__c === null || motorRequest.Motor__c === "") &&
        (motorRequest.Id === undefined || motorRequest.Id === null || motorRequest.Id === "")
      )
  ...
  actionHandler: function (component, action, parse, showIndicator, message) {
    ...
    if (showIndicator) self.functions.toggleSpinner(component, true, message);
    return new Promise(function (resolve, reject) {
      action.setCallback(self, function (response) {
        var state = response.getState();
        if (state === "SUCCESS") {
        ...
        if (showIndicator) self.functions.toggleSpinner(component, false);
      });
  ...
  selectTypeFunction: function (component, family, recordType) {
    ...
    return new Promise(function (resolve, reject) {
      ...
      self.fetchProducts(component, recordType, family).then(
        //fetchProducts SUCCESS
        $A.getCallback(function (response) {
          //self.functions.clearVars( component );
          self.handleConfigChange(component, { whatChanged: "reset" }).then(
  ...
  selectBoatFunction: function (component, boatId, selectedOptions) {
            ...
            productOptions.forEach((optionGroup) => {
              optionGroup.values.forEach((option) => {
                if (productId === option.id) {
                  if (options[productId].isCheckbox) option.isSelected = true;
                  else option.quantitySelected = options[productId].quantitySelected;
    ...
    return new Promise(function (resolve, reject) {
      if (boatId === undefined || boatId === null) resolve();
      else {
        helper
          .fetchProduct(component, boatId, "Boat")
          .then(
  ...
  handleTrailer: function (component, trailerId, selectedOptions) {
    ...
    return new Promise(function (resolve, reject) {
      if (trailerId == null || trailerId == "none") {
        ...
        self.handleConfigChange(component, changeData).then(function () {
          resolve(null);
        });
      } else {
        ...
        var trailerOptions = component.get("v.trailerSelectOptions"),
          ...
          feeData = [],
          trailerData;
        self.fetchProduct(component, trailerId, "Trailer").then(
          function (response) {
            ...
            let opts = [];
            try {
              opts = Object.keys(trailer.optionalProducts).reduce((result, key) => {
          ...
          */
            for (var i = 0; i < trailerOptions.length; i++) {
              if (trailerOptions[i].id === trailer.id) {
                ...
                if (trailerOptions[i].isUpgrade == true) trailerName = "Upgrade to ";
    ...
    return new Promise((resolve, reject) => {
      if (motorId == null || motorId == "none") {
        ...
        var motorOptions = component.get("v.motorSelectOptions"),
          ...
          feeData = [],
          motorData;
        self.fetchProduct(component, motorId, "Motor").then(
          function (response) {
            motor = JSON.parse(response);
            component.set("v.productSelected", true);
            if (motor.fees !== undefined && motor.fees !== null && !isMotorRequestOnly) {
              feeData = feeData.concat(self.handleFees(motor, component));
            }
            let opts = [];
            if (!isMotorRequestOnly) {
              try {
                opts = Object.keys(motor.optionalProducts).reduce((result, key) => {
          ...
          */
            if (
              isMotorRequestOnly ||
              (!isFactoryStore && order_ProductRecordType !== "Motor" && !dealerOrder.Is_Legend_Transfer__c)
            ) {
              motorCost = null;
            } else {
            /*
          ...
          */
            ...
            if (isMotorRequestOnly) changeData.cost_description = "Package and Save";
    ...
    return new Promise(function (resolve, reject) {
      if (trollingMotorId == null || trollingMotorId == "none") {
        ...
        var trollingMotorOptions = component.get("v.trollingMotorSelectOptions"),
          ...
          feeData = [],
          trollingMotorData;
        self.fetchProduct(component, trollingMotorId, "Trolling Motor").then(
          function (response) {
            ...
            let opts = [];
            try {
              opts = Object.keys(trollingMotor.optionalProducts).reduce((result, key) => {
  ...
  handleConfigChange: function (component, params) {
    var self = this,
      getProvincialFees = function (product) {
        let feeData = [];
        if (product.fees !== undefined && product.fees !== null) {
      ...
      processFees = function (params) {
        ...
        /* if changeData does not exist the whatChanged product is
             being removed
          */
        if (params.changeData === undefined && fees[params.whatChanged] !== undefined) {
          ...
          if (fees[params.whatChanged] !== undefined) delete fees[params.whatChanged];
    ...
    return new Promise(function (resolve, reject) {
      if (params.whatChanged === "reset") self.functions.clearConfigVars(component);
      ...
      if (params.whatChanged === "trailer") {
        if (params.changeData !== undefined) component.set("v.trailer", params.changeData);
        else component.set("v.trailer", []);

        processFees(params);
      }

      if (params.whatChanged === "motor") {
        if (params.changeData !== undefined) component.set("v.motor", params.changeData);
        else {
      ...
      if (params.whatChanged === "trolling motor") {
        ...
        } else component.set("v.trollingMotor", []);
      }
  ...
  checkForCanvasDiscount: function (component) {
    ...
    if (canvasDiscountAmount !== null) {
      ...
      if (hasCanvasOptions && discountIndex === undefined) {
        discounts.push({
          name: discountName,
          amount: canvasDiscountAmount * -1
        });
      } else if (!hasCanvasOptions && discountIndex !== undefined) {
  ...
  functions: {
    clearVars: function (component) {
      ...
      component.set("v.motorRequest", { sobjectType: "Dealer_Motor_Request__c", Quantity__c: 0, Motor__c: null });
