({
  afterScripts: function (component, event, helper) {
    helper.setUserData(component).then(
      $A.getCallback((response) => {
        var navMap = ["order-details", "build-boat", "finalize-container"];
  ...
  handleOrderCancel: function (component, event, helper) {
    ...
    helper.cancelOrder(component).then(
  ...
  nextStage: function (component, event, helper) {
    ...
    if (navMap.length === params.navigateTo) return false;
  ...
  previousStage: function (component, event, helper) {
    ...
    if (params.navigateTo < 0) return false;
