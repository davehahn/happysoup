({
  ...
  afterScripts: function (component, event, helper) {
    helper.setupUserAndPermissions(component).then(
      $A.getCallback(function (result) {
        ...
        if (result.warehouse !== null && result.warehouse !== undefined && result.warehouse.length > 0) {
  ...
  handleLocationChange: function (component, event, helper) {
    ...
    helper.removeAllEventSourcesByLocation(component).then(
  ...
  toggleEventSource: function (component, event, helper) {
    ...
    } else helper.removeEventSource(component, val);
  },
