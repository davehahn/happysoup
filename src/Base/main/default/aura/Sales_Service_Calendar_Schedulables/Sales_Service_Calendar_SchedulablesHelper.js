({
  loadSchedulable: function (component) {
    ...
    var self = this;
    self.fetchSchedulables(component).then(
  ...
  fetchSchedulables: function (component) {
    var action = component.get("c.fetchSchedulables"),
      la;
    action.setParams({
  ...
  findRecordsByCalendars: function (component) {
    ...
    for (let r of allRecords) {
      if (r.storeLocation.length === 0 && calTypes.indexOf(r.calId) > -1) toDisplay.push(r);
      else if (cals.indexOf(r.calId) > -1) toDisplay.push(r);
    }
    return toDisplay;
  },
  ...
  applySearch: function (component, searchValue) {
    ...
    for (let rec of displayedRecs) {
      if (rec.Name && rec.Name.toLowerCase().includes(searchValue)) result.push(rec);
      else if (rec.accountName && rec.accountName.toLowerCase().includes(searchValue)) result.push(rec);
      else if (rec.boatName && rec.boatName.toLowerCase().includes(searchValue)) result.push(rec);
    }
