({
  ...
  loadBalance: function (component, event, helper) {
    ...
    if (totalApplied == totalCanBeApplied * -1) return;
    else if (amountBCM >= totalCanBeApplied) toApply = totalCanBeApplied + totalApplied;
    ...
    if (!Array.isArray(getAllFields)) {
      ...
      for (var i = 0; i < getAllFields.length; i++) {
        ...
        console.log(cId);
        if (cId == inputId) $elem.set("v.value", toApply);
      }
    }
